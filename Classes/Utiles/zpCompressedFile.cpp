...
{
	assert(package != NULL);
	assert(!package->m_fileData.isNull());
	...
	memcpy((char*)m_chunkPos, m_package->m_fileData.getBytes() + (m_offset + 0), m_chunkCount * sizeof(u32));
	if (!checkChunkPos())
	{
...
u32 CompressedFile::oneChunkRead(u8* buffer, u32 size)
{
	...
	memcpy((char*)compressed, m_package->m_fileData.getBytes() + (m_offset + 0), m_compressedSize);
...
bool CompressedFile::readChunk(u32 chunkIndex, u32 offset, u32 readSize, u8* buffer)
{
	...
	if (compressedChunkSize == originChunkSize)
	{
		...
		memcpy((char*)dstBuffer, m_package->m_fileData.getBytes() + (m_offset + m_chunkPos[chunkIndex]), originChunkSize);
	}
	else
	{
		u8* compressed = new u8[compressedChunkSize];
		//fread((char*)compressed, compressedChunkSize, 1, m_package->m_stream);
		memcpy((char*)compressed, m_package->m_fileData.getBytes() + (m_offset + m_chunkPos[chunkIndex]), compressedChunkSize);
